...
Original file is located at
    https://colab.research.google.com/drive/1V7q8DhQU9x_sMopPHsq_ZE6Smaj7jw-X
"""

 # copy code lines from given text
import spacy
nlp = spacy.load('en_core_web_sm')
word1 = nlp("cat")
word2 = nlp("monkey")
word3 = nlp("banana")
word4 = nlp("onomatopoeia") # add my own example
...
tokens = nlp('cat apple monkey banana onomatopoeia') # add my own example
for token1 in tokens:
  for token2 in tokens:
    print(token1.text, token2.text, token1.similarity(token2))
print("\n")

 # banana and apple are most similar, which seems strange!
...
print("\n")
 ...
 # specifying the model we want to use. Remember to install this model by typing python -m spacy download en_core_web_md into your command line
...
'In a large bowl, cream together the butter, brown sugar, and white sugar. Beat in the instant pudding mix until blended. Stir in the eggs and vanilla. Blend in the flour mixture. Finally, stir in the chocolate chips and nuts. Drop cookies by rounded spoonfuls onto ungreased cookie sheets.'
 ...
 # We will now compare the similarity of the recipes. 
 # to ascertain how well spaCy's similarity
...
'''
Whenever I try to run spacy.load('en') or spacy.load('en_core_web_md'), 
it says both versions are outdated. I am only able to run spacy.load('en_core_web_sm'). 
Is there a new way to do this?
Also, I have to run Spacy on Google Colab because Replit doesn't load it properly?
